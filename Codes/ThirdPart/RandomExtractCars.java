...
//			.registerKryoClasses(new Class[]{SpeedSortKey.class})
			...
			MockData.mock(sc, spark); // 直接类名.方法名调用。用来模拟数据
		}else{ //否则就通过Hive模拟数据
		...
		//从配置文件中查询出来指定的任务ID
		long taskId = ParamUtils.getTaskIdFromArgs(args, Constants.SPARK_LOCAL_TASKID_EXTRACT_CAR);//args,"2"
		...
		Task task = taskDAO.findTaskById(taskId);//"2"对应的是{"startDate":["2018-07-01"],"endDate":["2018-07-01"],"extractNum":["100"]}
		...
		/**
		 * 统计出一段时间内所有卡扣的信息，所以需要根据param参数，去monitor_flow_action临时表中获取结果
		 * 返回的Row类型的RDD
		 */
		JavaRDD<Row> cameraRDD = SparkUtils.getCameraRDDByDateRange(spark, taskParamsJsonObject);

		/**
		 * 随机抽取N个车辆信息——一天有24个小时，其中08:00~09:00的车辆数量占当天总车流量的50%，在这天中共需要随机抽取100辆，
		 * 那么08:00~09:00的，就得抽取100*50%=50，而且这50个需要随机抽取。
		 * 使用Spark自己实现一个算法，按照时间段分段抽取车辆信息，这些车辆信息可以很权威的代表整个城市的车辆信息，
		 * 基于这些抽样的数据进行数据分析，可以绘制出这些车辆每天的运行轨迹，对于道路的规划起到了很重要的作用，
		 * 比如，抽样出来的数据80%的车辆在早高峰和晚高峰都是基本同样的行车轨迹，然而他们每天途径的路段都会堵车，这时候可以根据这些数据对道路进行规划
		 * 可以根据用户的画像进行多维度的数据分析
		 * 
		 * 下面方法中将抽取出来的车辆信息插入到random_extract_car表中,将抽取的car的详细数据放入random_extract_car_detail_info表中
		 * 返回(car,Row)
		 */
		 ...
		 * 相同的车辆会出现在不同的时间段中，可以追踪在这个日期段中车辆的行驶轨迹
		 */
	...
	private static JavaPairRDD<String, String> getCarTrack(long taskId, JavaPairRDD<String, Row> randomExtractCar2DetailRDD) {
		...
		JavaPairRDD<String, Iterable<Row>> groupByCar = randomExtractCar2DetailRDD.groupByKey();//图里有
		...
		JavaPairRDD<String, String> carTrackRDD = groupByCar.mapToPair(new PairFunction<Tuple2<String,Iterable<Row>>, String,String>() {
			...
			public Tuple2<String, String> call(Tuple2<String, Iterable<Row>> tuple) throws Exception {
				...
				String date = "";
				for (Row row : rows) {
					carTrack.append(","+row.getString(1)); // monitor_id
					date = row.getString(0);
				}
				return new Tuple2<String, String>(car, Constants.FIELD_DATE+"="+date+"|"+Constants.FIELD_CAR_TRACK+"="+carTrack.substring(1));//去掉逗号
			}
		});
	...
	private static JavaPairRDD<String, Row> randomExtractCarInfo(
		...
		JavaPairRDD<String, String> dateHourCar2DetailRDD = cameraRDD.mapToPair(
			...
			public Tuple2<String, String> call(Row row) throws Exception {
				String actionTime = row.getString(4);//2018-09-12 14:22:22
				String dateHour = DateUtils.getDateHour(actionTime);//2018-09-12_08
				String car = row.getString(3);//京X91427
				/**
				 * 为什么要使用组合Key？
				...
				// dateHour=2018-09-12_08, car=京Axxxxx
		 ...
		 */
		 JavaPairRDD<String, String> dateHour2DetailRDD = dateHourCar2DetailRDD.distinct(); //图里第一行dateHourCar2DetailRDD到dateHour2DetailRDD
		...
		/**String：日期
		 * Map<String,Long>：小时段，车辆数
		 * 将<dateHour,car_count>这种格式改成格式如下： <date,<Hour,count>>
		 */
		Map<String, Map<String,Long>> dateHourCountMap = new HashMap<>();
		// for循环完成了图里dateHourCountMap
		for (Entry<String, Long> entry : countByKey.entrySet()) { //entrySet即键值对集合
			String dateHour = entry.getKey();//2018-01-01_08
			String[] dateHourSplit = dateHour.split("_");////2018-01-01和08
			String date = dateHourSplit[0]; //2018-01-01
			String hour = dateHourSplit[1]; //08
			//本日期时段对应的车辆数
			Long count = Long.parseLong(String.valueOf(entry.getValue()));
			
			Map<String, Long> hourCountMap = dateHourCountMap.get(date); //get一个k，返回对应的v
		 ...
		 * 假设要抽取100辆车
		 */
		int extractNums = Integer.parseInt(ParamUtils.getParam(params, Constants.FIELD_EXTRACT_NUM)); //对应数据库extractNum字段下的100
		
		/**
		 * 一共抽取100辆车，平均每天应该抽取多少辆车呢？
		 * extractNumPerDay = 100 ， dateHourCountMap.size()是有多少不同的天数，
		 */
		int extractNumPerDay = extractNums / dateHourCountMap.size(); // 一开始功能要求仅1天，所以100/1
		
		/**
		 * 记录每天每小时抽取索引的集合
		 * dateHourExtractMap ---- Map<"日期"，Map<"1个小时段"，List<Integer>(抽取数据索引)>>
		 */
		...
		for (Entry<String, Map<String, Long>> entry : dateHourCountMap.entrySet()) {
			...
			//计算出这一天总的车流量
			long dateCarCount = 0L; //当前日期的总车辆数，初始化为0
			...
			for (Entry<String, Long> hourCountEntry : hourCountMap.entrySet()) {
				 ...
				 * 假设在一天中，7~8点这个时间段总车流量为100，假设之前算出在7~8点抽出的车辆数为20
				 * 怎么样随机抽取？
				 * 1.循环20次
				 * 2.每次循环搞一个0~100的随机数，放入一个list<Integer>中，这个list中的每一个元素就是car的index
				 * 3.最后得到一个长度为20的car的indexList<Integer>集合，一会取值取20个，取哪个值，取这里List中的下标对应的car
				 * 
				 */
				for(int i = 0 ; i < hourExtractNum ; i++){
					...
					while(extractIndexs.contains(index)){  //确保每天每个小时段抽到的索引下标不重复
...
//		} 
		...
		// 进来的是Tuple2<String,Tuple2<String,Row>>，两个String都是car
		// 返回的是String,Row，即car，Row
