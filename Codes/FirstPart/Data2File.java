...
/**
 * 本地运行的话用MockData.java
 * 集群运行要向Hive里传递文件，用本代码生成两个文件，即两张表的数据
 */
public class Data2File {
	...
	public static Boolean CreateFile(String pathFileName){
		try {
			// https://blog.csdn.net/qq_36894974/article/details/79046054
			File file = new File(pathFileName);
			if(file.exists()){
				file.delete();//exists、delete、createNewFile该3个方法都返回boolean值
			}
	...
	// https://www.cnblogs.com/yanpingping/p/10597419.html
	public static void WriteDataToFile(String pathFileName,String newContent,FileOutputStream fos,OutputStreamWriter osw ,PrintWriter pw) throws FileNotFoundException, UnsupportedEncodingException {
		...
		File file = new File(pathFileName);
		fos=new FileOutputStream(file,true);  // true数据从文件尾部写入；false数据覆盖原文件。
		osw=new OutputStreamWriter(fos, "UTF-8"); // 字节流转字符流
		pw =new PrintWriter(osw); // https://www.cnblogs.com/renzhe111/archive/2019/09/23/11572740.html
		pw.write(content+"\n");
	}
	...
	public static void mock() throws IOException {
    	...
    	String[] locations = new String[]{"鲁","川","渝","京","沪","贵","云","深","浙","粤"};
    	...
    	// https://blog.csdn.net/clear_love8/article/details/83792940
    	pw.flush(); // 将缓冲区的数据强制输出，用于清空缓冲区，若直接调用close()方法，则可能会丢失缓冲区的数据。即刷新的作用。
