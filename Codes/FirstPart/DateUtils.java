...
public class DateUtils {
    ...
    // static修饰后可以直接用，不用先创建对象。final修饰后不能更改。
    ...
    public static boolean before(String time1, String time2) {
        try {
            Date dateTime1 = TIME_FORMAT.parse(time1); //parse——将字符串格式转化为日期格式
    ...
    public static int minus(String time1, String time2) {
        try {
            ...
            long millisecond = datetime1.getTime() - datetime2.getTime(); //getTime()把日期转换为毫秒值（相当于System.currentTimeMillis()方法）

            // https://blog.csdn.net/weixin_37650458/article/details/85212730
            // Integer.valueOf是将String或者int类型转化为包装类型的Integer，所以要用String.valueOf走一遍
            return Integer.valueOf(String.valueOf(millisecond / 1000)); // 除以1000转换为秒
        } catch (Exception e) {
            e.printStackTrace();
        }
        return 0;
...
//		 String second = dateTime.split(" ")[1].split(":")[2];
		 if(minute+(5-minute % 5) == 60){ // 55，56，57，58，59满足
